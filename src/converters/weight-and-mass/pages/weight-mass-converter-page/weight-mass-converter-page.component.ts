...
export class WeightMassConverterPageComponent extends PageBase implements OnInit {
  // Valores selecionados
  categoriaSelecionada: string = 'peso-massa';
  unidadeOrigemSelecionada: string = 'grama';
  unidadeDestinoSelecionada: string = 'quilograma';
  ...
  constructor(
    meta: Meta,
    title: Title,
    private weightMassService: WeightMassConverterService,
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      if (conversionParam) {
        const conversionInfo = this.weightMassService.parseConversionUrl(conversionParam);
        if (conversionInfo) {
          ...
          this.router.navigate(['/conversores/peso-massa']);
        }
      } else {
        this.gerarConversoesSugeridas();
      }
  ...
  private atualizarURL(): void {
    const url = this.weightMassService.generateConversionUrl(
      this.unidadeOrigemSelecionada,
      this.unidadeDestinoSelecionada
    );

    // Atualiza a URL sem recarregar a página
    this.router.navigate(['/conversores/peso-massa', url], { replaceUrl: true });
  }

  private atualizarTitulo(): void {
    const unidadeOrigem = this.weightMassService.getUnitById(this.unidadeOrigemSelecionada);
    const unidadeDestino = this.weightMassService.getUnitById(this.unidadeDestinoSelecionada);
...
2. Converter ${valorBaseStr} ${unidadeBase.symbol} para ${unidadeDestino.name}:
  ...
  /**
    ...
    * Usa gramas como unidade base padrão
    */
  private obterUnidadeBase(): CalculatorUnit | null {
    return this.weightMassService.getUnitById('grama') ?? null;
  }
